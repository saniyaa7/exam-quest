...
const Login = () => {
  ...
  else if(userType==='BOS')
    x='BOARD OF STUDIES';
  else if(userType==='COE')
    x='CONTROLLER OF EXAMINATION';
    ...
    if (errors.email === '' && errors.password === '') {
      axios
        ...
        .then((res) => {
          if (res.data === 'Success') {
           if(userType==='AD')
              navigate('/Branch_wise?userType=AD');
            else if(userType==='BOS')
            navigate('/sem?database=CSE');
            else if(userType==='COE')
            navigate('/coe');
      ...
      <div className="login_box">
        <div className="left">
          ...
          <div className="contact">
           ...
           <p className='link'>Don't have an account?<Link to={`/signup?userType=${userType}`}>SignUp</Link></p>
